...
这个入口会转到静默启动器；正常情况下**不会长期停留 cmd / PowerShell 窗口**，桌宠会在后台启动，并通过**系统托盘图标**提供显示/隐藏与其他操作入口。
...
>
> 运行时你主要会在 **系统托盘** 和 **任务管理器的后台进程** 中看到它；Windows 是否把托盘图标常驻显示在任务栏外侧，仍取决于你的系统托盘设置。
...
```text
dist-release/cline-desktop-pet-0.1.0-win-portable.zip
```

解压后运行 `Cline Desktop Pet.exe` 即可启动。便携版仍会把本地数据保存到 `%APPDATA%/cline-desktop-pet/`，并通过系统托盘提供显示、隐藏、诊断和退出入口。
